...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MrBot1000 v2.0.21")
        ...
        with open(req_path, encoding="utf-8") as fh:
            for line in fh:
                ...
                if not line or line.startswith("#") or line.startswith("-"):
                    continue
                # strip version specifiers: "pkg>=1.2" -> "pkg"
    ...
    def _validate_model_config(self, main_model: str | None, chat_model: str | None):
        ...
        def warn(msg):
            self.thought_panel.route("System", f"[Startup] MODEL WARN: {msg}")
            print(f"[Startup] MODEL WARN: {msg}")
        ...
        if main_model and chat_model and main_model.strip() == chat_model.strip():
            warn("OLLAMA_MAIN_MODEL == OLLAMA_CHAT_MODEL. Co-running identical models "
                 "on a single GPU (e.g. GTX 1660S) can starve VRAM/latency — use a "
                 "smaller/CPU chat model.")
        # Heuristic: chat model looks like a large main-class model (>=~9B).
        big = ("9b", "13b", "14b", "27b", "30b", "32b", "70b", "120b", "E2B", "qwen3", "llama3.1")
        if chat_model and any(tok in chat_model.lower() for tok in big):
            warn(f"OLLAMA_CHAT_MODEL '{chat_model}' looks like a large model — chat may "
                 f"be slow/VRAM-heavy. A ~1B chat model (e.g. gemma-3-1b) is recommended.")
    ...
    def create_ui(self):
        ...
        title_lbl = QLabel("MrBot1000 v2.0.21")
